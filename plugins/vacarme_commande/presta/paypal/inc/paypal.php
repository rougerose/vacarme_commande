...
function bank_paypal_recoit_notification(){
	...
	if (!$row = sql_fetsel("*","spip_commandes_transactions",
	...
	if ($id = sql_getfetsel("id_transaction","spip_commandes_transactions","autorisation_id=".sql_quote($autorisation_id)." AND mode='paypal'")){
	...
	sql_updateq("spip_commandes_transactions",array("autorisation_id"=>$autorisation_id,"mode"=>'paypal'),"id_transaction=".intval($id_transaction));
